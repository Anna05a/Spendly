...
class StatisticsPageView(View):
    ...
    def get(self, request):
        try:
            ...
            statements = mono.get_statements(self.request.session['card_id'],
            ...
            for payment in payments:
                ...
                context = {'finances': finances, 'data': data, 'expence_persent': expence_percent,
                           'revenue_persent': revenue_percent, 'expences': total_spending, 'payments': total_payments,
                           'revenue_data': total_r, 'expense_data': total_e}
            return render(self.request, 'main/statistic_page.html', context)
        ...
        if user_info:
            for user_account in user_info['accounts']:
                ...
                if not Card.objects.filter(
                        card_id=encrypted_card_id).exists():  # Перевіряємо, чи ідентифікатор уже існує
                    card_obj = Card.objects.create(id=uuid.uuid4(), card_id=encrypted_card_id, balance=card_balance,
                                                   card_number=card_number, user=user, token=token, type=type,
                                                   system=card_type)
